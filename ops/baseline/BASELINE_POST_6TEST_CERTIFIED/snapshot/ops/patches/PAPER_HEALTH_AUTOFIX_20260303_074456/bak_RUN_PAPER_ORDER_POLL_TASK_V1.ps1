...
$__OUT = Join-Path $__APPEND_DIR "ORDER_POLL_OUT_APPEND.txt"
$__ERR = Join-Path $__APPEND_DIR "ORDER_POLL_ERR_APPEND.txt"
...
$out = Join-Path $ops ("ORDER_POLL_OUT_{0}.txt" -f $ts)
$err = Join-Path $ops ("ORDER_POLL_ERR_{0}.txt" -f $ts)

& "C:\Program Files\PowerShell\7\pwsh.exe" -NoProfile -ExecutionPolicy Bypass `
  -File (Join-Path $ProjectRoot "tools\paper\POLL_PAPER_ORDERS_V1.ps1") `
  -ProjectRoot $ProjectRoot -RunRoot $RunRoot `
  1> $out 2> $err
