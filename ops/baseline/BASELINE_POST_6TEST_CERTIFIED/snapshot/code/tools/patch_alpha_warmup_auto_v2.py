...
# Replace the V1 injected block with a V2 block (safe: locate TAG1 block start)
...
# remove the V1 block heuristically until the blank line after the injected return
end = None
for j in range(start, min(len(lines), start+80)):
    if "return type('AlphaDecision'" in lines[j]:
        # remove until after the following blank line
...
# detect indentation from start line
