...
function EnsureFile($p){ EnsureDir (Split-Path $p -Parent); if(!(Test-Path $p)){ "" | Set-Content -Encoding UTF8 -Path $p } }
function Finger([string]$s){ if([string]::IsNullOrWhiteSpace($s)){""} else { $n=$s.Length; $s.Substring(0,[Math]::Min(4,$n))+"****"+$s.Substring([Math]::Max(0,$n-4)) } }
...
try{
  # load secrets safely (fingerprint only)
  $ldr = Join-Path $ProjectRoot "tools\secrets\LOAD_PROFILE_SECRETS_V1.ps1"
  if(Test-Path $ldr){
    . $ldr -Profile $Profile -ProjectRoot $ProjectRoot -RunRoot $RunRoot | Out-Null
  }
  ...
  # --- build required artifacts in backup dir ---
  ...
  # KPI json (minimal but audit-friendly)
  $meta = Join-Path $RunRoot "logs\meta.jsonl"
  $ledger = Join-Path $RunRoot "logs\ledger"
  $obj = [ordered]@{
    ts=$ts; ymd=$ymd; profile=$Profile; runroot=$RunRoot
    zip=$zip.Name
    key_id_fingerprint=(Finger ([string]$env:APCA_API_KEY_ID))
    meta_exists=(Test-Path $meta)
    meta_bytes=(if(Test-Path $meta){ (Get-Item $meta).Length } else { 0 })
    ledger_exists=(Test-Path $ledger)
    ledger_files=(if(Test-Path $ledger){ (Get-ChildItem $ledger -File -ErrorAction SilentlyContinue).Count } else { 0 })
  }
  ($obj | ConvertTo-Json -Depth 10) | Set-Content -Encoding UTF8 -Path $kpiJson

  # REALIZED KPI placeholder (later می‌تونی با parser واقعی پرش کنی)
  ([ordered]@{ ts=$ts; ymd=$ymd; profile=$Profile; realized_pnl_usd=$null; notes="placeholder" } |
  ...
  try{
    foreach($p in $req){
      ...
      $old=$z.Entries | ?{ $_.FullName -eq $name } | Select -First 1
      if($old){ $old.Delete() }
      [System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile($z,$p,$name) | Out-Null
    }
  } finally { $z.Dispose() }
  Add-Content -Encoding UTF8 -Path $out -Value "ZIP_QC_KPI_POLICY_INJECTED=1"

  # Final enforce
  $stillMissing = @($req | ?{ !(Test-Path $_) })
  if($stillMissing.Count -gt 0){
    Add-Content -Encoding UTF8 -Path $err -Value ("QC_ENFORCE_FAIL missing=" + (($stillMissing | % {Split-Path $_ -Leaf}) -join ","))
    if($KillOnFail -eq 1){
