...
# RUN_SHADOW_PROFILE_CANON_V5 (direct python launcher; observable; exits 0)
...
# defaults (از روی commandlineهای قبلی خودت)
...
# write header to LATEST (do NOT overwrite with empty stamp later)
@(
  ("ts=" + (Get-Date).ToString("s")),
  "PROFILE=SHADOW",
  ("RUNROOT=" + $RunRoot),
...
# confirm process exists; if not => fail start + show err tail
...
if(!$p2){
  ...
  if(Test-Path $leStamp){
    $tail = Get-Content $leStamp -ErrorAction SilentlyContinue | Select -Last 40
    if($tail){ $tail | Add-Content -Encoding UTF8 -Path $leLatest }
  }
