...
function SafeCopy([string]$src, [string]$dst){
  if(Test-Path $src){
    EnsureDir (Split-Path $dst -Parent)
...
if(Test-Path $KDocs){
  Copy-Item -Recurse -Force $KDocs (Join-Path $bakDir "docs") -ErrorAction SilentlyContinue
}
...
# --- Build DAILY zips (each must contain LIVE_ERR evidence even if empty)
foreach($d in $Days){
  ...
  ) | Set-Content -Encoding UTF8 -Path $man

  # Zip daily (stored in dailies/)
...
if($takeN -lt 1){ throw "No daily zips found in: $KDailies" }
...
# --- Minimal stage-gate docs + journal template
$sg1 = @"
...
# --- Task proofs (best effort; non-fatal)
...
# --- Rebuild SUPER zip (docs + rolling + static zips if present)
