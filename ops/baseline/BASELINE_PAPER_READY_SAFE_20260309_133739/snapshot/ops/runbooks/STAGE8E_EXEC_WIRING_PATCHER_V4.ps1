...
$PDIR=Join-Path $ROOT ("ops\patches\STAGE8E_EXEC_WIRING_V4_" + $stamp)
$ADIR=Join-Path $ROOT ("ops\audit\STAGE8E_EXEC_WIRING_V4_" + $stamp)
...
foreach($EXE in $targets){
  ...
  $lines = $raw -split "`r`n"
  if(-not $lines -or $lines.Count -eq 0){ throw ("SPLIT_LINES_EMPTY=" + $EXE) }
...
try {
  $rr0 = $env:RUNROOT; if([string]::IsNullOrWhiteSpace($rr0)){ $rr0="C:\alpaca-bot\ORG_BOT_UNIFIED\runtime\paper" }
  STAGE8E-CallLedgerPy -Action "reconcile" -RunRoot $rr0 -Rid "RID:RECON" | Out-Null
} catch {}
# === STAGE8E_EXEC_WIRING_END ===
'@

  # Insert wiring after param() block if present
  $insertAt = 0
  if($lines.Count -gt 0 -and ($lines[0].TrimStart().ToLower().StartsWith("param("))){
    $close=-1
    for($i=0;$i -lt $lines.Count;$i++){ if($lines[$i].Trim() -eq ")"){ $close=$i; break } }
    if($close -gt -1){ $insertAt = $close + 1 }
  }

  $outLines=@()
  for($i=0;$i -lt $lines.Count;$i++){
    if($i -eq $insertAt){ $outLines += ($wire -split "`r`n") }
    $outLines += $lines[$i]
  }
  if($insertAt -ge $lines.Count){ $outLines += ($wire -split "`r`n") }

  if(-not $outLines -or $outLines.Count -eq 0){ throw ("OUTLINES_EMPTY=" + $EXE) }
  ...
  # Apply inserts (now guaranteed Lines is string[])
