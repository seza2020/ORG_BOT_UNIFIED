...
if($targets.Count -eq 0){ throw ("NO_TARGETS_FOUND: " + $PAPER) }

$stamp=Get-Date -Format "yyyyMMdd_HHmmss"
$PDIR=Join-Path $ROOT ("ops\patches\STAGE8E_EXEC_WIRING_V8_" + $stamp)
$ADIR=Join-Path $ROOT ("ops\audit\STAGE8E_EXEC_WIRING_V8_" + $stamp)
New-Item -ItemType Directory -Force -Path $PDIR | Out-Null
New-Item -ItemType Directory -Force -Path $ADIR | Out-Null

function Coerce-ToLines {
  param([object]$x)
  if($null -eq $x){ return @() }
  if($x -is [string[]]){ return $x }
  if($x -is [array]){ return @($x) }
  if($x -is [string]){ if([string]::IsNullOrWhiteSpace($x)){ return @() }; return ($x -split "`r`n") }
  return @([string]$x)
}
...
function Insert-AfterFirstMatch {
  ...
  for($i=0;$i -lt $Lines.Count;$i++){
    $out += $Lines[$i]
    if(-not $done -and ($Lines[$i] -match $Pattern)){ $out += $Block; $done=$true }
  }
...
function Save-Evidence {
  param([string]$ExePath,[string]$Raw,[string]$Reason)
  try { $Reason | Set-Content (Join-Path $ADIR ("reason_" + [IO.Path]::GetFileName($ExePath) + ".txt")) -Encoding utf8 } catch {}
  try { $Raw    | Set-Content (Join-Path $ADIR ("raw_"    + [IO.Path]::GetFileName($ExePath) + ".txt")) -Encoding utf8 } catch {}
  try {
    ($Raw | Select-String -Pattern "ConvertFrom-Json|order_submitted|order_error|dryrun_skip_submit|kind\s*=|order_intent" -AllMatches |
      Select-Object LineNumber,Line | Format-Table -AutoSize | Out-String) |
      Set-Content (Join-Path $ADIR ("hits_" + [IO.Path]::GetFileName($ExePath) + ".txt")) -Encoding utf8
  } catch {}
}
...
foreach($EXE in $targets){
  try {
    ...
    $lines = Coerce-ToLines ($raw -split "`r`n")
    if($lines.Count -eq 0){ throw ("SPLIT_LINES_EMPTY=" + $EXE) }
...
'@
    ...
    $outp = & $pyExe @("-c", $py) 2>&1 | Out-String
    return @{ ok=$true; out=$outp }
  } catch { return @{ ok=$false; out=$_.Exception.Message } }
}

try {
  $rr0 = $env:RUNROOT
  if([string]::IsNullOrWhiteSpace($rr0)){ $rr0="C:\alpaca-bot\ORG_BOT_UNIFIED\runtime\paper" }
  STAGE8E-CallLedgerPy -Action "reconcile" -RunRoot $rr0 -Rid "RID:RECON" | Out-Null
} catch {}
...
'@
    ...
    $outLines=@()
    for($i=0;$i -lt $lines.Count;$i++){ if($i -eq $insertAt){ $outLines += ($wire -split "`r`n") }; $outLines += $lines[$i] }
    if($insertAt -ge $lines.Count){ $outLines += ($wire -split "`r`n") }
    $outLines = Coerce-ToLines $outLines
    if($outLines.Count -eq 0){ throw ("OUTLINES_EMPTY_AFTER_WIRE=" + $EXE) }

    $reserveHook=@(
      "# === STAGE8E_RESERVE_BEFORE_SUBMIT BEGIN ===",
      "try {",
      "  `$rr = `$env:RUNROOT; if([string]::IsNullOrWhiteSpace(`$rr)){ `$rr = `"$RUNROOT_DEFAULT`" }",
    ...
    $outLines = Insert-AfterFirstMatch -Lines $outLines -Pattern "ConvertFrom-Json" -Block $reserveHook -Required
    $outLines = Insert-AfterFirstMatch -Lines $outLines -Pattern 'kind\s*=\s*"order_submitted"' -Block $commitHook -Required
    $outLines = Insert-AfterFirstMatch -Lines $outLines -Pattern 'kind\s*=\s*"order_error"' -Block $releaseHook -Required
    $outLines = Insert-AfterFirstMatch -Lines $outLines -Pattern 'kind\s*=\s*"dryrun_skip_submit"' -Block $dryHook

    ($outLines -join "`r`n") | Set-Content -LiteralPath $EXE -Encoding utf8
    Unblock-File -LiteralPath $EXE -ErrorAction SilentlyContinue
    ("PATCH_OK=" + $EXE) | Out-Host
  } catch {
    ("PATCH_FAIL=" + $EXE) | Out-Host
    ("ERR=" + $_.Exception.Message) | Out-Host
    Save-Evidence -ExePath $EXE -Raw $raw -Reason $_.Exception.Message
    throw
  }
}
