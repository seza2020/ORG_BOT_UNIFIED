...
$mgrLog  = Join-Path $LOGS "manager_hard_v5_events.jsonl"
$mgrLock = Join-Path $LOCKDIR "ORG_UNIFIED_RUNTIME_MANAGER_HARD_V5.lock"
...
function Snapshot-Procs([string]$PathOut) {
  $pyProcs = Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
    Where-Object {
      $_.CommandLine -and
      (
        $_.CommandLine -match "tbot\.entrysafe" -or
...
try {
  ...
  try {
    $live = Snapshot-Procs -PathOut (Join-Path $OutDir "cert\processes_final.txt")
