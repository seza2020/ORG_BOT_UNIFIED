...
$OUT  = Join-Path $EVID ("OPS_RUNTIME_OWNER_GUARD_FIX_V1_CERTIFY_" + $TS)
...
$opsGuard   = Join-Path $LOGS "ops_runtime_owner_guard.log"
...
foreach ($f in @($metaEvents,$metaEngine,$managerLog,$launcherLog,$opsGuard)) {
...
@(
  "OPS_RUNTIME_OWNER_GUARD_FIX_V1_CERTIFY_DONE"
  ("OUT=" + $OUT)
  ("COUNT_AFTER_12S=" + $c12)
  ("COUNT_AFTER_20S=" + $c20)
