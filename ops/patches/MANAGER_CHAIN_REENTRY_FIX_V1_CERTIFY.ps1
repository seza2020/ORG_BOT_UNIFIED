...
$OUT  = Join-Path $EVID ("MANAGER_CHAIN_REENTRY_FIX_V1_CERTIFY_" + $TS)
...
$managerGrowth    = $afterManager - $beforeManager

foreach ($f in @($metaEvents,$metaEngine,$launcherLog,$managerLog,$paperStdout,$paperStderr)) {
...
@(
  "MANAGER_CHAIN_REENTRY_FIX_V1_CERTIFY_DONE"
  ("OUT=" + $OUT)
  ...
  ("MANAGER_LOG_GROWTH=" + $managerGrowth)
