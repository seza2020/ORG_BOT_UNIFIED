...
$OUT  = Join-Path $EVID ("MANAGER_STDOUT_FORENSIC_V1_CERTIFY_" + $TS)
...
$mgrStdout  = Join-Path $LOGS "manager_python_stdout.log"
$mgrStderr  = Join-Path $LOGS "manager_python_stderr.log"
...
$beforeStdout  = Get-LineCount -Path $mgrStdout
$beforeStderr  = Get-LineCount -Path $mgrStderr
...
$afterStdout  = Get-LineCount -Path $mgrStdout
$afterStderr  = Get-LineCount -Path $mgrStderr
...
foreach ($f in @($managerLog,$mgrStdout,$mgrStderr,$launcherLog,$metaEvents,$metaEngine)) {
...
@(
  "MANAGER_STDOUT_FORENSIC_V1_CERTIFY_DONE"
  ("OUT=" + $OUT)
  ("COUNT_AFTER_12S=" + $count12)
  ("COUNT_AFTER_20S=" + $count20)
  ("MANAGER_LOG_GROWTH=" + ($afterManager - $beforeManager))
  ("MANAGER_STDOUT_GROWTH=" + ($afterStdout - $beforeStdout))
  ("MANAGER_STDERR_GROWTH=" + ($afterStderr - $beforeStderr))
