...
$mgrLog   = Join-Path $LOGS "manager_hard_v61_events.jsonl"
$mgrLock  = Join-Path $LOCKDIR "ORG_UNIFIED_RUNTIME_MANAGER_HARD_V61.lock"
...
try {
  ...
  while ($true) {
    ...
    if (-not $ownerAlive -and -not $certReached) {
  ...
  if ($count12 -lt 0 -and (Test-Path (Join-Path $OutDir "cert\processes_t12.txt"))) {
    $count12 = (Get-Content -LiteralPath (Join-Path $OutDir "cert\processes_t12.txt") | Select-String "ProcessId").Count
  }
  if ($count20 -lt 0 -and (Test-Path (Join-Path $OutDir "cert\processes_t20.txt"))) {
    $count20 = (Get-Content -LiteralPath (Join-Path $OutDir "cert\processes_t20.txt") | Select-String "ProcessId").Count
  }
