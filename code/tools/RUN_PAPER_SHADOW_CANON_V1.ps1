param(
  ...
  [string]$ProfilePath = "C:\alpaca-bot\ORG_BOT_UNIFIED\code\tools\profiles\paper.profile.json",
  [int]$Force = 0
)
...
# STEP63_ABSOLUTE_LOADER_PATH_V1
$CanonicalSecretsLoader = "C:\alpaca-bot\ORG_BOT_UNIFIED\code\tools\secrets\LOAD_PROFILE_SECRETS_V1.ps1"
# STEP53_SECRETS_PROJECTROOT_FIX_V1
$SecretsProjectRoot = $ProjectRoot
try {
  if (-not [string]::IsNullOrWhiteSpace([string]$SecretsProjectRoot)) {
    if ((Split-Path $SecretsProjectRoot -Leaf).ToLower() -eq 'code') {
      $SecretsProjectRoot = Split-Path $SecretsProjectRoot -Parent
    }
  }
} catch {}

# STEP57_SECRETS_LOADER_PATH_FIX_V1
$SecretsLoaderPath = $CanonicalSecretsLoader
# STEP49_PROFILE_BOOTSTRAP_FIX_V1
$ProjectRoot = "C:\alpaca-bot\ORG_BOT_UNIFIED\code"
$ProfilePath = "C:\alpaca-bot\ORG_BOT_UNIFIED\code\tools\profiles\paper.profile.json"
...
try{
  ...
  $ldr = $CanonicalSecretsLoader
  if(!(Test-Path $ldr)){ throw ('MISSING_LOADER=' + $ldr) }

  . $ldr -Profile $exp -ProjectRoot $SecretsProjectRoot -RunRoot $RunRoot | Out-Null
...
try{
  ...
  $ldr = $CanonicalSecretsLoader
  . $ldr -Profile $prof -ProjectRoot $SecretsProjectRoot -RunRoot $RunRoot | Out-Null
} catch { throw }
# DEDUP_SECRETS_MARKER_PROFILE_SECRETS_LOAD_V1 (commented): # PROFILE_SECRETS_LOAD_V1 (PAPER)
try{
  & ($CanonicalSecretsLoader) -Profile "PAPER" -ProjectRoot $SecretsProjectRoot -RunRoot $RunRoot | Out-Null
}catch{ throw }

# CONFIG_GATES_RUNNER_V1
& "C:\Program Files\PowerShell\7\pwsh.exe" -NoProfile -ExecutionPolicy Bypass -File "C:\alpaca-bot\ORG_BOT_UNIFIED\code\tools\ops\VALIDATE_PAPER_CONFIG_GATES_V1.ps1" -ProjectRoot "C:\alpaca-bot\ORG_BOT_UNIFIED\code" -RunRoot "$RunRoot"
...
try {
  ...
  # Precheck (safe)
  pwsh -NoProfile -ExecutionPolicy Bypass -File ("C:\alpaca-bot\ORG_BOT_UNIFIED\code\tools\PRECHECK_PAPER_PROFILE_V1.ps1") `
...
# Paths inside RunRoot
  ...
  "-m","tbot.main",
  "--run",
  "--iters", ([string]($prof.args.iters)),
  "--sleep", ([string]($prof.args.sleep))
)
  if([bool]$a.shadow){
    # STEP79_REMOVED_UNSUPPORTED_SHADOW_ARGS
  }

  # STEP79_REMOVED_UNSUPPORTED_GATE_ARGS
...
Start-Sleep -Seconds 3
  ...
  $aliveP = Get-CimInstance Win32_Process -Filter "ProcessId = $($p.Id)" -ErrorAction SilentlyContinue
  if($null -eq $aliveP){
    Write-Host ("[ANTI_SELF_SPAWN] FAIL: launched pid not alive -> EXIT 97 pid={0}" -f $p.Id)
    exit 97
  }
